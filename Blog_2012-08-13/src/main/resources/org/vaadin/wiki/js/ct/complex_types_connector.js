window.org_vaadin_wiki_js_ct_ComplexTypesComponent = function() {

	var connectorId = this.getConnectorId();
	var element = this.getElement();

	this.registerRpc({
		sendComplexTypes : function(list, stringMap, otherMap, connectorMap,
				bits, matrix, bean) {
			var message = 'list[2]  = "' + list[2] + '"<br />';
			message += 'stringMap.two = ' + stringMap.two + '<br />';
			message += 'otherMap[1][1] = "' + otherMap[1][1] + '"<br />';
			message += 'connectorMap[connectorId] = "'
					+ connectorMap[connectorId] + '"<br />';
